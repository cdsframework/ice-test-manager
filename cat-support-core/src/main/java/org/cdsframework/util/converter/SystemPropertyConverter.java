...
@Named
@RequestScoped
public class SystemPropertyConverter extends BaseDTOConverter<SystemPropertyDTO> {

    @Inject
    private SystemPropertyDTOList systemPropertyDTOList;

    public SystemPropertyConverter() {
        super(SystemPropertyConverter.class);
    }

    @Override
    protected void initialize() {
        setBaseDTOList(systemPropertyDTOList);
    }

    @Override
    protected String getStringFromObject(SystemPropertyDTO item) {
        return item != null ? item.getPropertyId() : null;
    }
}
