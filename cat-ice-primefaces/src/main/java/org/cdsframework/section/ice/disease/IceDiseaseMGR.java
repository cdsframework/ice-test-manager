 ...
 */
package org.cdsframework.section.ice.disease;
...
@Named
@ViewScoped
public class IceDiseaseMGR extends BaseModule<IceDiseaseDTO> {

    private static final long serialVersionUID = 2784912296773467972L;

    @Override
    protected void initialize() {
        setLazy(true);
        setSaveImmediately(true);
        setBaseHeader("Disease");
    ...
    public void onSearchDialogReturn(SelectEvent selectEvent) throws Exception {
        ...
        IceDiseaseDTO parentDTO = getParentDTO();
        logger.info(METHODNAME, "parentDTO=", parentDTO);
        String targetFieldName = getTargetFieldName();
        logger.info(METHODNAME, "targetFieldName=", targetFieldName);

        if (baseDTO instanceof CdsListItemDTO) {
            CdsListItemDTO cdsListItemDTO = (CdsListItemDTO) baseDTO;
            if (selectedDTO instanceof IceDiseaseVersionConceptRelDTO) {
                OpenCdsConceptDTO openCdsConceptDTO = cdsListItemDTO.getOpenCdsConceptDTO();
                IceDiseaseVersionConceptRelDTO iceDiseaseVersionConceptRelDTO = (IceDiseaseVersionConceptRelDTO) selectedDTO;
                if (openCdsConceptDTO != null) {
                    iceDiseaseVersionConceptRelDTO.setPrimaryOpenCdsConceptDTO(openCdsConceptDTO);
                }
            } else if (parentDTO != null) {
                CdsCodeDTO cdsCodeDTO = cdsListItemDTO.getCdsCodeDTO();
                parentDTO.setDiseaseCdsCodeDTO(cdsCodeDTO);
            }
        } else if (baseDTO instanceof CdsVersionDTO) {
            CdsVersionDTO cdsVersionDTO = (CdsVersionDTO) baseDTO;
            if (selectedDTO instanceof IceDiseaseVersionConceptRelDTO) {
                IceDiseaseVersionConceptRelDTO iceDiseaseVersionConceptRelDTO = (IceDiseaseVersionConceptRelDTO) selectedDTO;
                iceDiseaseVersionConceptRelDTO.setCdsVersionDTO(cdsVersionDTO);
            }
        }
    }
